...
const Gwangju = () => {
    ...
    useEffect(() => {
      ...
      console.log("광주주 범죄 데이터 가져오기 시작...");
  
      Promise.all(
        crimeTypes.map((type) =>
          fetch(`https://port-0-smap-backend-main-mhkpzrkrde061e33.sel3.cloudtype.app/areaGraph/search?region=광주&crimetype=${encodeURIComponent(type)}`)
            .then((res) => res.json())
            .then((data) => {
